...
#        user.set_db(self.db, 'user')        
        users = self.db_session.query(UserN).all()
        
                
        if len(users) == 0:
            # add default user
            print 'no user(s) found, creating default root user'
            u = UserN('root', 'root', sha1('debug').hexdigest(), 0, 1000, True,
                      False, True, 'default', 'default', 'default', UserN.F_CHARGE | UserN.F_CREATE_USER | UserN.F_CREATE_CHARGE)
            session.add(u)
            users = self.db_session.query(UserN).all()
    ...
    def processor_adduser(self, u, query):
        ...
        new_user = UserN(username, desc, p, u.uid, 0, can_weblogin, can_post, need_check, cm, cu, ct, flags)
...
#        new_user.save()
    ...
    def processor_manageuser(self, u, query):
        ...
        d = None
        if (ext != None and ext != ''):
            d = session.query(UserN.uid).filter('ext = :vale').paramter(value=ext).all()
